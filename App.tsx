...
const getLeaveTheme = (type: LeaveType) => {
  switch (type) {
    ...
    case LeaveType.PUBLIC_HOLIDAY: return { color: 'bg-indigo-500', bg: 'bg-indigo-50', text: 'text-indigo-600', icon: Flag, label: 'วันหยุดนักขัตฯ' };
    case LeaveType.MATERNITY: return { color: 'bg-purple-500', bg: 'bg-purple-50', text: 'text-purple-600', icon: Baby, label: 'ลาคลอด' };
    case LeaveType.LEAVE_WITHOUT_PAY: return { color: 'bg-slate-600', bg: 'bg-slate-50', text: 'text-slate-700', icon: Wallet, label: 'ลาไม่รับเงินเดือน' };
...
const StatusBadge = ({ status }: { status: LeaveStatus }) => {
  ...
  return <span className={`px-2 py-0.5 rounded-full text-[9px] font-black border uppercase tracking-widest ${styles[status]}`}>{status}</span>;
};
...
const App: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) { alert('ส่งคำขอสำเร็จ'); fetchData(user!); setView('dashboard'); setNewReq({type: LeaveType.ANNUAL, startDate: '', endDate: '', reason: '', attachment: ''}); }
    setLoading(false);
  };
        ...
        <div className="bg-slate-50 rounded-[2rem] p-8 border border-slate-100 flex flex-col items-center gap-6">
          <div className="relative">
            <div className="w-24 h-24 rounded-3xl overflow-hidden border-4 border-white shadow-xl">
              {linePicture ? <img src={linePicture} className="w-full h-full object-cover" /> : <User className="w-full h-full p-6 text-slate-300" />}
            </div>
          ...
          <div className="w-full space-y-4">
            <div className="relative">
              ...
              <input value={staffIdInput} onChange={e=>setStaffIdInput(e.target.value)} className="w-full bg-white pl-12 pr-4 py-4 rounded-2xl font-bold border border-slate-100 outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="Staff ID" />
            </div>
            <button onClick={handleLogin} disabled={loading} className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl shadow-xl shadow-blue-100 active:scale-95 transition-all flex items-center justify-center gap-2">
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 shadow-2xl flex flex-col pb-20">
      {/* Header */}
      <header className="bg-white px-6 py-4 flex justify-between items-center sticky top-0 z-40 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="font-black text-slate-800 text-lg leading-none">LMS</h1><p className="text-[9px] font-bold text-blue-500 uppercase tracking-widest mt-1">{user?.siteId}</p></div>
        </div>
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        {view === 'dashboard' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <section className="flex justify-between items-end">
              <div><h2 className="text-2xl font-black text-slate-800">สวัสดี, {user?.name.split(' ')[0]}</h2><p className="text-slate-400 font-bold text-xs uppercase tracking-tight">{user?.position}</p></div>
              <button onClick={()=>setView('new')} className="bg-blue-600 text-white p-4 rounded-2xl shadow-2xl shadow-blue-200 active:scale-90 transition-all"><PlusCircle size={28} /></button>
            </section>

            <section className="grid grid-cols-2 gap-4">
              {balances.map(b => {
                const theme = getLeaveTheme(b.type);
                const progress = (b.used / (b.used + b.remain)) * 100;
                return (
                  <div key={b.type} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                    <div className="flex justify-between items-start">
                      <div className={`p-2.5 rounded-xl ${theme.bg} ${theme.text}`}><theme.icon size={20} /></div>
                      <ChevronRight size={14} className="text-slate-200" />
                    </div>
                    <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 truncate">{theme.label}</p><h4 className="text-2xl font-black text-slate-800">{b.remain}<span className="text-[10px] ml-1 text-slate-300">วัน</span></h4></div>
                    <div className="w-full h-1 bg-slate-50 rounded-full overflow-hidden"><div className={`h-full ${theme.color}`} style={{width: `${progress}%`}} /></div>
                  </div>
                );
              })}
            </section>
            ...
            {user?.roleType === UserRole.SUPERVISOR && requests.some(r => r.status === LeaveStatus.PENDING && r.staffId !== user.staffId) && (
              <section className="bg-blue-600 p-6 rounded-[2.5rem] shadow-xl text-white space-y-6">
                <div className="flex items-center gap-2"><ShieldCheck size={20} /><h3 className="font-black text-sm uppercase tracking-widest">คำขอรออนุมัติ ({requests.filter(r => r.status === LeaveStatus.PENDING && r.staffId !== user.staffId).length})</h3></div>
                  ...
                  {requests.filter(r => r.status === LeaveStatus.PENDING && r.staffId !== user.staffId).map(req => (
                    <div key={req.id} className="bg-white/10 backdrop-blur-md rounded-3xl p-5 border border-white/20">
                      <div className="flex justify-between items-start mb-4">
                        <div><p className="text-[10px] font-black text-blue-100 uppercase tracking-widest">{req.staffName}</p><h4 className="font-bold text-sm">{getLeaveTheme(req.type).label}</h4><p className="text-[10px] text-blue-200">{req.startDate} - {req.endDate} ({req.totalDays} วัน)</p></div>
                        {req.attachmentUrl && <button onClick={()=>setZoomImg(req.attachmentUrl!)} className="bg-white/20 p-2 rounded-xl text-white"><Search size={16} /></button>}
                      </div>
                      <div className="flex gap-2">
                        <button onClick={()=>handleAction(req.id, LeaveStatus.APPROVED)} className="flex-1 bg-white text-blue-600 font-black py-2.5 rounded-xl text-[10px] uppercase shadow-lg shadow-black/10">Approve</button>
                        <button onClick={()=>handleAction(req.id, LeaveStatus.REJECTED)} className="flex-1 bg-rose-500 text-white font-black py-2.5 rounded-xl text-[10px] uppercase shadow-lg shadow-black/10">Reject</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section>
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 mb-4">รายการล่าสุด</h3>
              <div className="space-y-3">
                {requests.filter(r => r.staffId === user?.staffId).slice(0, 3).map(req => (
                  <div key={req.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-xl ${getLeaveTheme(req.type).bg} ${getLeaveTheme(req.type).text}`}><Calendar size={18} /></div>
                      <div><h5 className="font-bold text-xs">{getLeaveTheme(req.type).label}</h5><p className="text-[10px] text-slate-400">{req.startDate}</p></div>
                    </div>
        ...
        {view === 'new' && (
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
            <header className="flex items-center gap-4"><button onClick={()=>setView('dashboard')} className="p-2 bg-white rounded-full shadow-sm text-slate-400"><ChevronLeft size={24} /></button><h2 className="text-xl font-black text-slate-800">ยื่นใบลาใหม่</h2></header>
              ...
              {requests.filter(r => r.staffId === user?.staffId).map(req => (
                <div key={req.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
                  ...
                  {req.status === LeaveStatus.REJECTED && (
                    <button onClick={()=>resubmit(req)} className="w-full bg-blue-50 text-blue-600 font-black py-3 rounded-xl text-[10px] uppercase active:scale-95 transition-all">ยื่นใหม่อีกครั้ง</button>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-[3rem] p-10 shadow-xl border border-white flex flex-col items-center">
              <div className="relative mb-6">
                <div className="w-32 h-32 rounded-[2.5rem] overflow-hidden border-4 border-white shadow-2xl">
                  {linePicture ? <img src={linePicture} className="w-full h-full object-cover" /> : <User className="w-full h-full p-8 text-slate-100" />}
                </div>
              ...
              <button onClick={()=>setIsLoggedIn(false)} className="w-full mt-10 bg-rose-50 text-rose-500 font-black py-5 rounded-3xl flex items-center justify-center gap-3 active:scale-95 transition-all"><LogOut size={20} /> ออกจากระบบ</button>
            </div>
          </div>
        )}
      </main>
      ...
      {zoomImg && (
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-6 animate-in fade-in duration-300" onClick={()=>setZoomImg(null)}>
          ...
          <img src={zoomImg} className="max-w-full max-h-full rounded-2xl shadow-2xl" />
        </div>
      )}
    </div>
  );
